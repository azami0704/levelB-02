...
<body>
	...
	<div id="all">
		<div id="title">
			<?=date("m 月 d 日 l")?> | 今日瀏覽:<?=$Total->sum('total',['date'=>date("Y-m-d")])?> | 累積瀏覽: <?=$Total->sum('total',1)?> <a href="index.php" class="float-r">回首頁</a></div>
		<div id="title2">
			...
			<div class="hal" id="main">
				<div>
					...
					if(isset($_SESSION['ac'])){
						echo "<div>歡迎，{$_SESSION['ac']}</div>";
						if($_SESSION['ac']=='admin'){
							echo "<a href='index.php' class='button'>管理</a>|";
						}
						echo "<a href='./api/logout.php' class='button'>登出</a>";
					}else{
						echo "<a href='?do=user'>會員登入</a>";
					}
